...
Devvit.addMenuItem({
  label: `Ninigram #29: Morning Pick-Me-Up! (Easy)`,
  location: 'subreddit',
  onPress: async (_event, context) => {
    ...
    try {
      const post = await reddit.submitPost({
        title: `Ninigram #29: Morning Pick-Me-Up! (Easy)`,
        subredditName: subreddit.name,
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #29...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(29));
      ui.showToast({ text: `Created Ninigram #29!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #29: ${error}` });
    }
  },
});

Devvit.addMenuItem({
  label: `Ninigram #30: Nutty or Nice? (Medium)`,
  location: 'subreddit',
  onPress: async (_event, context) => {
    const { reddit, ui, kvStore } = context;
    const subreddit = await reddit.getCurrentSubreddit();
    ...
    try {
      const post = await reddit.submitPost({
        title: `Ninigram #30: Nutty or Nice? (Medium)`,
        subredditName: subreddit.name,
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #30...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(30));
      ui.showToast({ text: `Created Ninigram #30!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #30: ${error}` });
    }
  },
});

Devvit.addMenuItem({
  label: `Ninigram #31: Hoof Hoof (Medium)`,
  location: 'subreddit',
  onPress: async (_event, context) => {
    const { reddit, ui, kvStore } = context;
    const subreddit = await reddit.getCurrentSubreddit();
    ...
    try {
      const post = await reddit.submitPost({
        title: `Ninigram #31: Hoof Hoof (Medium)`,
        subredditName: subreddit.name,
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #31...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(31));
      ui.showToast({ text: `Created Ninigram #31!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #31: ${error}` });
    }
  },
});
  ...
  onPress: async (_event, context) => {
    ...
    try {
      const post = await reddit.submitPost({
        ...
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #32...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(32));
      ui.showToast({ text: `Created Ninigram #32!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #32: ${error}` });
    }
  },
});
  ...
  onPress: async (_event, context) => {
    ...
    try {
      const post = await reddit.submitPost({
        ...
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #33...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(33));
      ui.showToast({ text: `Created Ninigram #33!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #33: ${error}` });
    }
  },
});
  ...
  onPress: async (_event, context) => {
    ...
    try {
      const post = await reddit.submitPost({
        ...
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
            <text size="large">Loading Ninigram #34...</text>
          </vstack>
        )
      });

      await kvStore.put(`puzzle_${post.id}`, String(34));
      ui.showToast({ text: `Created Ninigram #34!` });
      ui.navigateTo(post);
    } catch (error) {
      ui.showToast({ text: `Failed to create Ninigram #34: ${error}` });
    }
  },
});
...
Devvit.addMenuItem({
  label: `Ninigram #3 (Remake): Field Duty (Easy)`,
  location: 'subreddit',
  onPress: async (_event, context) => {
    const { reddit, ui, kvStore } = context;
    const subreddit = await reddit.getCurrentSubreddit();
    ...
    try {
      const post = await reddit.submitPost({
        title: `Ninigram #3 (Remake): Field Duty (Easy)`,
        subredditName: subreddit.name,
        preview: (
          <vstack height="100%" width="100%" alignment="middle center">
